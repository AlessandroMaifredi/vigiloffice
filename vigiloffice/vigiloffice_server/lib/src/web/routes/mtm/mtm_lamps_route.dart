...
class JsonLampsRoute extends WidgetRoute {
  ...
  Future<WidgetJson> _get(Session session, HttpRequest request) async {
    ...
    session.log('Found ${lamps.length} lamps.', level: LogLevel.debug);
    request.response.headers.contentType = ContentType.json;
    request.response.statusCode = HttpStatus.ok;
    setHeaders(request.response.headers);
  ...
  Future<WidgetJson> _post(Session session, HttpRequest request) async {
    ...
    session.log('MTM | Received POST request with body: $content',
        level: LogLevel.debug);
    try {
      LampsEndpoint endpoint = LampsEndpoint();
      Lamp lamp = Lamp.fromJson(json.decode(content));
      if ((await endpoint.readLamp(session, lamp)) != null) {
        request.response.statusCode = HttpStatus.conflict;
        request.response.reasonPhrase = 'Lamp already exists.';
        request.response.headers.contentType = ContentType.json;
        setHeaders(request.response.headers);
        return WidgetJson(object: {'error': 'Lamp already exists.'});
      }
      lamp = await endpoint.createLamp(session, lamp);
      session.log('MTM | Lamp created: ${lamp.macAddress}',
          level: LogLevel.debug);
      request.response.statusCode = HttpStatus.ok;
      request.response.headers.contentType = ContentType.json;
      setHeaders(request.response.headers);
      ...
      setHeaders(request.response.headers);
      return WidgetJson(
          object: {'error': 'Failed to create lamp. ${e.toString()}'});
    }
  }
}
